...
const chapterPattern =
  /^(#{1,2}\s*)?((chapter|cap[ií]tulo)\s+([0-9]+|[ivxlcdm]+|one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|uno|dos|tres|cuatro|cinco|seis|siete|ocho|nueve|diez)(\s*[:.-]\s*.*)?|prologue|pr[oó]logo|epilogue|ep[ií]logo|introduction|introducci[oó]n|afterword|[0-9]+\.\s+.+|[0-9]+\s*:\s+.+)$/imu;
...
function splitChapters(text: string): ParsedChapter[] {
  ...
  lines.forEach((line, index) => {
    ...
    if (trimmed.length <= 90 && chapterPattern.test(trimmed)) {
