...
BookForge starts from a different premise: **a book is a structure, not a blob of text.** Projects contain books. Books contain chapters, scenes, and paragraphs. Every revision is a versioned, reversible event — never an overwrite. Every AI call carries the *whole-book* context it needs: the Manuscript Blueprint (a generated book bible of characters, themes, voice, and rules), nearby chapter summaries, continuity ledgers, locked passages, and the author's own boundaries. The author accepts or rejects every change. Nothing ships to the manuscript without a human decision.
