...
- Added initial course read surfaces: `/courses` catalog and `/courses/[courseId]` detail pages with module/lesson/asset visibility and source-book linking.
- Added course freshness policy coverage on course catalog/detail screens (`12h/24h` and `6h/12h`) plus navigation shortcuts from app shell and dashboard finished-book cards.
...
4. Continue admin/course domain implementation from ADR-0001 (enrollments/progress/admin controls).
